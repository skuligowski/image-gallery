...
function LazyLoadingGrid<T>({ render, items }: SourceableProps<T>): React.ReactElement<any, any> {
  ...
  const isBottom = (el: HTMLElement): boolean => Math.abs(el.scrollHeight - el.scrollTop - el.clientHeight) < 200;
  const noScroll = (el: HTMLElement): boolean => el.scrollHeight === el.clientHeight;

  const handleScroll: React.UIEventHandler<HTMLElement> = (e) => !loaded && isBottom(e.currentTarget) && next();
  const handleFillUp = (el: HTMLElement) => !loaded && (noScroll(el) || isBottom(el)) && next();
